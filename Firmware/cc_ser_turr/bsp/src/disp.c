...
/* List of Display Data Bits GPIO Port Bits */
static GPIO_TypeDef *DISP_DB_PORT[DISP_MAX_NB_BITS] =
{
  DISP_DB0_PORT,
  DISP_DB1_PORT,
  DISP_DB2_PORT,
...
/* List of Display Data Bits GPIO Port Pins */
static const uint8_t DISP_DB_PIN[DISP_MAX_NB_BITS] =
{
  DISP_DB0_PIN,
  DISP_DB1_PIN,
  DISP_DB2_PIN,
...
/* List of Display Enable GPIO Port Pins */
...
@inline static void WriteToDisplay(uint8_t dispNo, volatile uint8_t data)
{
  ...
  /* Write Byte on Port */
	for(bit = 0; bit < DISP_MAX_NB_BITS; bit++)
	{
		if(data & (1 << bit))
		{
			DISP_DB_PORT[bit]->ODR |= (uint8_t)DISP_DB_PIN[bit];
		}
		else
		{
			DISP_DB_PORT[bit]->ODR &= (uint8_t)~DISP_DB_PIN[bit];
