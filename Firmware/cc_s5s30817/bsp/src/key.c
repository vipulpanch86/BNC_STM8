...
#define MAX_RT_LINES   (5)

#define NB_RT_4_SC0    (5)
...
(NB_RT_4_SC0 + NB_RT_4_SC1 + NB_RT_4_SC2 + NB_RT_4_SC3 + \
 NB_RT_4_SC4 + NB_RT_4_SC5 + NB_RT_4_SC6 + NB_RT_4_SC7 + \
 NB_RT_4_SC8)
...
/**
  ...
  * @retval None
  */
#define SetAllScanLines() { \
    MUX1_MSA_PORT->ODR &= (uint8_t)~MUX1_MSA_PIN; \
    MUX1_MSB_PORT->ODR &= (uint8_t)~MUX1_MSB_PIN; \
    MUX1_MSC_PORT->ODR &= (uint8_t)~MUX1_MSC_PIN; \
    MUX1_MEN_PORT->ODR &= (uint8_t)~MUX1_MEN_PIN; \
}
...
#define ClrScanLine(nb) { \
  if(nb < SC_NB_MUXED) { \
    ((nb) & 0x01) ? (MUX1_MSA_PORT->ODR |= (uint8_t)MUX1_MSA_PIN) : (MUX1_MSA_PORT->ODR &= (uint8_t)~MUX1_MSA_PIN); \
    ((nb) & 0x02) ? (MUX1_MSB_PORT->ODR |= (uint8_t)MUX1_MSB_PIN) : (MUX1_MSB_PORT->ODR &= (uint8_t)~MUX1_MSB_PIN); \
    ((nb) & 0x04) ? (MUX1_MSC_PORT->ODR |= (uint8_t)MUX1_MSC_PIN) : (MUX1_MSC_PORT->ODR &= (uint8_t)~MUX1_MSC_PIN); \
    MUX1_MEN_PORT->ODR |= (uint8_t)MUX1_MEN_PIN; \
  } \
}
...
#define GetReturnVal(val) { \
  val = (uint8_t)((KEY_RET0_PORT->IDR & KEY_RET0_PIN) ? 1<<0 : val); \
  val = (uint8_t)((KEY_RET1_PORT->IDR & KEY_RET1_PIN) ? 1<<1 : val); \
}
/**
...
void KPD_Scan(void)
{
  ...
  for(keyScanNo = 0; keyScanNo < NB_KEYS; keyScanNo++)
  {
    ...
    //keyPressState = (keyPressBit & (1 << keyScanNo)) ? CLOSED : OPEN;
